...
//   styleUrls: ['./interview-schedule.scss'],
...
@Component({
  ...
  imports: [CommonModule, FormsModule, MatCardModule,
    ...
    MatFormFieldModule, MatDatepickerModule,
    MatNativeDateModule, MatSelectModule, MatMenuModule,
    MatToolbarModule, MatDividerModule, InitialsPipe, MatInputModule],
  providers: [DatePipe],
  animations: [
    trigger('detailExpand', [
      state('collapsed', style({ height: '0px', minHeight: '0', display: 'none' })),
...
export class InterviewSchedule implements OnInit {
  ...
  displayedInterviewColumns: string[] = [
    'interviewId',
    'roundName',
    'interviewDate',
    'feedback',
  ...
  viewResume(candidate: any) {
    if (candidate.resume) {
      const fileUrl = `${candidate.resume}`;
